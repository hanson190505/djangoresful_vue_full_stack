<template>
  ...
  <el-dialog
    title="新增其他属性"
    v-model="selectMoreTypeFormVisible"
    append-to-body
  >
    <el-form
      :model="selectMoreTypeForm"
      ref="selectMoreTypeFormRef"
      label-width="100px"
      :rules="rules"
    >
      <el-form-item label="属性名称" prop="label">
        <el-input
          v-model="selectMoreTypeForm.label"
          autocomplete="off"
        ></el-input>
      </el-form-item>
      <el-form-item label="内容格式" prop="type">
        <el-select
          v-model="selectMoreTypeForm.type"
          placeholder="内容格式"
          clearable
        >
...
export interface IselectMoreTypeForm {
  label: string;
  element: string | [] | number | IMoreDict | { [k: string]: string };
  span: number;
  type: string;
}
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const rules = {
      ...
      type: [{ required: true, message: '请选择内容格式', trigger: 'change' }],
    };
        ...
        if (valid) {
          switch (selectMoreTypeForm.type) {
            case 'dict':
              ...
              selectMoreTypeForm.span = 12;
              break;
            case 'list':
              selectMoreTypeForm.element = [''];
              selectMoreTypeForm.span = 8;
              break;
            case 'input':
              selectMoreTypeForm.element = '';
              selectMoreTypeForm.span = 8;
